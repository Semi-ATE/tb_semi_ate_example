...
If you have the need to add things, add it to 'Instrumentinit.py' or 'common.py'
...
class Instrumentinit_OP:
    """Class definition for the output parameters of Instrumentinit."""

    def __init__(self):
        self.num_outputs = 0
        self.result = OutputParameter('result', -inf, 0.0, 0.0, 0.0, inf, 0, False)
        self.result.set_format('.3f')
        self.result.set_unit('˽')
        self.temperature = OutputParameter('temperature', -inf, -45.0, 0.0, 170.0, inf, 0, False)
        self.temperature.set_format('.3f')
        self.temperature.set_unit('˽')
...
class Instrumentinit_IP:
    """Class definition for the input parameters of Instrumentinit."""
...
class Instrumentinit_BC(DutTestCaseBase):
    '''Base class definition for Instrumentinit'''
    ...
    def __init__(self, instance_name: str, sbin, test_num, context, stil_tool: StilToolBase):
        ...
        self.ip = Instrumentinit_IP()
        self.op = Instrumentinit_OP()
    ...
    def aggregate_test_result(self, site_num: int, exception: bool):
        ...
        if not exception:
            ...
            current_result = self.op.temperature.get_testresult()
            test_result = self._select_testresult(test_result, current_result)
            test_bin = self._select_bin(test_bin, current_result)
            stdf_data.append(self.op.temperature.generate_stdf_result_record(current_result[0], site_num))
        else:
            ...
            test_result = Result.Fail()
            test_bin = self.get_sbin()
            stdf_data.append(self.op.temperature.generate_stdf_result_record(test_result, site_num))
    ...
    def aggregate_tests_summary(self, head_num: int, site_num: int):
        ...
        stdf_data.append(self.op.temperature.generate_tsr_record(head_num, site_num, self.get_average_test_execution_time()))
